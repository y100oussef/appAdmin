...
import { Plus, Pencil, Upload, Trash2, Download } from "lucide-react";
import {
  Card,
  CardContent,
  CardDescription,
...
const Wallpapers = () => {
  ...
  // Add new function to export wallpapers as JSON
  const handleExportJSON = () => {
    try {
      // Create a JSON file with the wallpapers data
      ...
      link.href = url;
      link.download = `wallpapers_export_${new Date().toISOString().split('T')[0]}.json`;
      ...
      toast.success("تم تصدير بيانات الخلفيات بنجاح");
    } catch (error) {
      console.error("Error exporting wallpapers:", error);
      toast.error("حدث خطأ أثناء تصدير بيانات الخلفيات");
    }
  };
